...
public class OinvAdvSN implements IAdvSN<OinvView> {
	 
	private Object[][] ob;
	private String hql;
	private AboutView av;
	@Override
    public void add(OinvView v,String SN,boolean ifdraft,String objtype,String Direction,boolean ifpasn,int rowid)
    {
		...
		{			
			if(!new BigDecimal(sns.getLength()).setScale(2, BigDecimal.ROUND_HALF_UP).equals(new BigDecimal(0).setScale(2, BigDecimal.ROUND_HALF_UP))
			 ...
			 &&v.getOd().getValuethrheader(j, "仓库代码")!=null&&v.getOd().getValuethrheader(j, "仓库代码").toString().equals(sns.getWareHouse()))
			{										
    ...
    public boolean verification(OinvView v)
    {
    	//check  snstatus
		...
		if(ob!=null&&ob.length>0)
		{
			...
			for(int i=0;i<v.getDsv().getOd().getRowCount();i++)
			{
				...
				if(rows>0)
				{
					JOptionPane.showMessageDialog(null,"非仓库组操作员不允许在备货单上输入序列号");
					return false;							
				}
			}
			if(rows==0)
	    ...
	    for(int i=0;i<v.getOd().getRowCount();i++)
		{
	    	...
	    	if(v.getOd().getValuethrheader(i, "物料代码")==null||(v.getOd().getValuethrheader(i, "物料代码")!=null&&v.getOd().getValuethrheader(i, "物料代码").toString().equals(""))
	    	  ||(ob!=null&&!ob[0][0].toString().equals("Y")))
			{
				 continue;
			}
	    	Integer gs=0;
	    	...
	    	if(!(gs==Integer.valueOf(new BigDecimal(v.getOd().getValuethrheader(i, "包装数量").toString()).setScale(0, BigDecimal.ROUND_HALF_UP).toString())
	    	  &&zl.setScale(3, BigDecimal.ROUND_HALF_UP).equals(new BigDecimal(v.getOd().getValuethrheader(i, "数量").toString()).setScale(3, BigDecimal.ROUND_HALF_UP))))
	    	{
	...
	@Override
	public boolean snverification(OinvView v) {
	    ...
	    {		    
	       ...
	       if(sns==null)
	       {
				...
				av.getTxtpnQserpDevelopedBy().setText(a[i]+"不存在，请检查");
	    	   return false;
	       }
	        /*Highlighter highlighter=null;
	    	highlighter=v.getJta_SN().getHighlighter();
	...
	@Override
	public boolean bfcverification(OinvView v) {
		// TODO Auto-generated method stub
		//check  snstatus
		String s =new String(v.getJta_SN().getText());   
		Pattern pat = Pattern.compile("\\s*|\t|\r|\n"); 		   
