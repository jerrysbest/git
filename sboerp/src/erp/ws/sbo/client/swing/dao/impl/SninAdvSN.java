...
public class SninAdvSN implements IAdvSN<SninView> {
    ...
    {		
		...
		for(int i=0;i<v.getOd().getRowCount();i++)
		{
			if(!new BigDecimal(sns.getLength()).setScale(2, BigDecimal.ROUND_HALF_UP).equals(new BigDecimal(0).setScale(2, BigDecimal.ROUND_HALF_UP))
			 ...
			 &&new BigDecimal(v.getOd().getValuethrheader(i, "米段").toString()).setScale(3, BigDecimal.ROUND_HALF_UP).equals(new BigDecimal(sns.getLength()).setScale(3, BigDecimal.ROUND_HALF_UP))
			...
			{		
				...
				v.getOd().setValuethrheader(((ComboBoxItem)v.getCom_whsin().getSelectedItem()).getValue().toString(), i, "仓库");
				warehouse=v.getOd().getValuethrheader(i, "仓库").toString();
			...
			{		
				Double sl=Double.valueOf(v.getOd().getValuethrheader(i, "实际库存数量").toString());				
				v.getOd().setValuethrheader(Integer.valueOf(new BigDecimal(v.getOd().getValuethrheader(i, "实际收货个数").toString()).setScale(0, BigDecimal.ROUND_HALF_UP).toString())+1,i,"实际收货个数");					
				v.getOd().valueChanged(i, v.getOd().getcolumnindex("实际收货个数"), v.getOd().getValuethrheader(i, "物料代码").toString(), "");
				v.getOd().setValuethrheader(sl+sns.getCweight(), i, "实际库存数量");	
    ...
    public void delete(DeSNView v,String SN)
    {
		...
		{
			if(!new BigDecimal(sns.getLength()).setScale(2, BigDecimal.ROUND_HALF_UP).equals(new BigDecimal(0).setScale(2, BigDecimal.ROUND_HALF_UP))
			 &&((SninView)v.getV()).getOd().getValuethrheader(i, "物料代码")!=null&&((SninView)v.getV()).getOd().getValuethrheader(i, "物料代码").toString().equals(sns.getItemcode())
			 &&new BigDecimal(((SninView)v.getV()).getOd().getValuethrheader(i, "米段").toString()).setScale(2, BigDecimal.ROUND_HALF_UP).equals(new BigDecimal(sns.getLength()).setScale(2, BigDecimal.ROUND_HALF_UP))
			 &&(((SninView)v.getV()).getOd().getValuethrheader(i, "是否米段线").toString().equals("Y")||((SninView)v.getV()).getOd().getValuethrheader(i, "是否米段线").toString().equals("是")))
			{										
				((SninView)v.getV()).getOd().setValuethrheader(Integer.valueOf(((SninView)v.getV()).getOd().getValuethrheader(i, "实际收货个数").toString())-1,i,"实际收货个数");	
				Double sl=Double.valueOf(((SninView)v.getV()).getOd().getValuethrheader(i, "实际库存数量").toString());
				((SninView)v.getV()).getOd().valueChanged(i,((SninView)v.getV()).getOd().getcolumnindex("实际收货个数"), ((SninView)v.getV()).getOd().getValuethrheader(i, "物料代码").toString(), "");
				((SninView)v.getV()).getOd().setValuethrheader(sl-sns.getCweight(), i, "实际库存数量");	
				break;				
			}
			else if(new BigDecimal(sns.getLength()).setScale(2, BigDecimal.ROUND_HALF_UP).equals(new BigDecimal(0).setScale(2, BigDecimal.ROUND_HALF_UP))
					 &&((SninView)v.getV()).getOd().getValuethrheader(i, "物料代码")!=null&&((SninView)v.getV()).getOd().getValuethrheader(i, "物料代码").toString().equals(sns.getItemcode())
					 &&(((SninView)v.getV()).getOd().getValuethrheader(i, "是否米段线").toString().equals("N")||((SninView)v.getV()).getOd().getValuethrheader(i, "是否米段线").toString().equals("否")))
			{						
				((SninView)v.getV()).getOd().setValuethrheader(Integer.valueOf(((SninView)v.getV()).getOd().getValuethrheader(i, "实际收货个数").toString())-1,i,"实际收货个数");	
				Double sl=Double.valueOf(((SninView)v.getV()).getOd().getValuethrheader(i, "实际库存数量").toString());
				((SninView)v.getV()).getOd().valueChanged(i, ((SninView)v.getV()).getOd().getcolumnindex("实际收货个数"), ((SninView)v.getV()).getOd().getValuethrheader(i, "物料代码").toString(), "");
