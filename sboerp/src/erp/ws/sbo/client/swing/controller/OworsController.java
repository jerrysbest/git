	  ...
	  if(e.getSource()==v.getCom_form()&&v.getOd1().ds.getCnValue().equals("增加"))
	  {
	   ...
	   // something about doctitle
		...
		{  
			...
			{			
				 try{
						docf.getDoc().setValues(v,docf.getDoc().getprev(Integer.valueOf(v.getTxt_docn().getText())),"草稿");			
					}
		...
		{  
			...
			{			
				 try{
					 docf.getDoc().setValues(v,docf.getDoc().getnext(Integer.valueOf(v.getTxt_docn().getText())),"草稿");			
					}
				catch(NumberFormatException e0){
