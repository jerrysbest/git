...
public class OOignsController  implements MouseListener,TableModelListener,
ListSelectionListener,InternalFrameListener,ActionListener,KeyListener {
	...
	private DaoFactory<OOignView> docf=(DaoFactory<OOignView>)appMain.ctx.getBean("OOignFactory");	
	public OOignsController(OOignView v) {
		...
		if(e.getKeyCode()==10)
		{
			if(e.getSource()==v.getJta_SN())
			{
				...
				if(docf.getAdnSN().snverification(v))
				{
				    ...
				    {
				      docf.getAdnSN().add(v, a[i], false, "59", "I", false,0);
				    }
				}
	...
	public void keyReleased(KeyEvent e) {
		...
		{ 
			...
			for(int k=0;k<i.length;k++)
			{
			 ...
			 {
				 JOptionPane.showMessageDialog(null, "ooignscontroller-keyreleased-127");
			     System.out.println("ooignscontroller-keyreleased-127");
			 }			
			}
		    v.getOd().delRow(i);		   
		    try{
	...
	public void actionPerformed(ActionEvent e) {
		...
		{					
			...
			QueryWindowView<OOignView> qwv=new QueryWindowView<OOignView>(v,docf.getQDoc(),v.getOd1(),hql,hql1);
			v.getParent().add(qwv);
			qwv.setVisible(true);
		...
		else if(e.getSource()==dmv.getjButtondelrow())
		{
			...
			for(int k=0;k<i.length;k++)
			{
			 ...
			 {
				 JOptionPane.showMessageDialog(null, "ooignscontroller-actionPerformed-delrow");
			     System.out.println("ooignscontroller-actionPerformed-delrow");
			 }
		...
		{			
			docf.getDoc().setValues(v, docf.getDoc().getfirst(),"生产收货");	
		...
		{  
			...
			{	
				...
				try{
					docf.getDoc().setValues(v,docf.getDoc().getprev(Integer.valueOf(v.getTxt_docn().getText())),"生产收货");
					}
		...
		{  
			...
			{			
				...
				try{
					docf.getDoc().setValues(v,docf.getDoc().getnext(Integer.valueOf(v.getTxt_docn().getText())),"生产收货");
					}
				catch(NumberFormatException e0){
		...
		{   		   
			docf.getDoc().setValues(v, docf.getDoc().getlast(),"其它入库");		
				...
				if(v.getOd1().ds.getCnValue().equals("查询"))
				{
					hql="select 0,a.ifdraft,a.objtype,a.docentry,a.linenum,a.sn,a.itemcode,a.length,a.weight,a.direction,a.ifpasn,a.pasn," +
						...
						"cdatetime=convert(nvarchar(10),a.cdatetime,23),udatetime=convert(nvarchar(10),a.udatetime,23) " +
				   		...
				   		"where c.objtype='59' and c.docnum='"+v.getTxt_docn().getText().trim()+"' and a.objtype='59'" +
	...
	public void tableChanged(TableModelEvent e) {
        ...
        for(int i=0;i<v.getDsv().getOd().getRowCount();i++)
        {
        	...
        	{
        	   v.getDsv().getOd().setValuethrheader(v.getOd().getValuethrheader(lastRow, "仓库"), i, "仓库");      
        	}
        }
